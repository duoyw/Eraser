...
class Expr:
    ...
    def getValue(self, value_expr):
        if "A_Const" in value_expr:
            ...
            if len(value_expr["TypeCast"]['typeName']['TypeName']['names']) == 1:
                ...
                if value_expr["TypeCast"]['typeName']['TypeName']['typemod'] == 2:
    ...
    def getAliasName(self, ):
        ...
        if len(self.expr["ColumnRef"]["fields"]) == 1:
            column_name = self.expr["ColumnRef"]["fields"][0]["String"]["str"]
    ...
    def __str__(self, ):
        if self.isCol():
            if len(self.expr["ColumnRef"]["fields"]) == 1:
                column_name = self.getAliasName()
                index = column_name.find("_")
                alias_name = column_name[:index]
...
class Comparison:
    def __init__(self, comparison):
        ...
        if "A_Expr" in self.comparison:
            ...
            # if len(self.lexpr.expr["ColumnRef"]["fields"]) == 1:
            #     column_name = self.lexpr.getAliasName()
            #     index = column_name.find("_")
            #     alias_name = column_name[:index]
            #     return alias_name+"."+column_name
            # else:
            #     return self.lexpr.getAliasName()+"."+self.lexpr.getColumnName()
            ...
            if self.lexpr.isCol():
                if len(self.lexpr.expr["ColumnRef"]["fields"]) == 1:
                    ...
                    column_name = self.lexpr.getColumnName()
                    alias_name = self.lexpr.getAliasName()

                self.aliasname_list.append(alias_name)
            ...
            if self.rexpr.isCol():
                ...
                if len(self.rexpr.expr["ColumnRef"]["fields"]) == 1:
                    column_name = self.rexpr.getAliasName()
                    index = column_name.find("_")
                    alias_name = column_name[:index]
                else:
                    column_name = self.rexpr.getColumnName()
                    alias_name = self.rexpr.getAliasName()
