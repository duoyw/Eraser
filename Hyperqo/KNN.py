...
class KNN:
    def __init__(self,k_neighbours,method='brute',max_distance=0.05**2+0.05**2+0.05**2+10,max_v=4):
        ...
        if method =='kd-tree':
            pass
    # (0.518774151802063, 0.5837623148482997, 1.0)
    # (0.3029925227165222, 0.818091847248532, 1.0)
    ...
    def kNeighbours(self,v,k_neighbours=0):
        ...
        # chosen_data = [(0.12833211156463187, 0.45664092898368835)]
        # v = (0.518774151802063, 0.5837623148482997, 1.0)
        # self.kvs =[((0.3029925227165222, 0.818091847248532, 1.0), 0.04200747728347776)]
        chosen_data = sorted([(self.distance(v,x[0]),x[1]) for x in self.kvs],key = lambda x:x[0])[:k_neighbours]
        if len(self.kvs)<k_neighbours:
            return []
        return chosen_data
    # v = [(0.8306915163993835, 1.3287104494339836, 1.0)] 
