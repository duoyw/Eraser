...
class Sql2Vec:
    ...
    def to_vec(self, sql):
        ...
        ###
        if config.database == 'tpch':
            alias_list = []
            for fullname in self.aliasnames_root_set:
                alias_list.append(config.tpch_map[fullname])
            self.aliasnames_root_set = set(alias_list)
        ###
        ...
        for table in self.from_table_list:
            ...
            if config.database == 'tpch':
                alias = config.tpch_map[alias]
                table.from_table['alias']['Alias']['aliasname'] = config.tpch_map[
                    table.from_table['alias']['Alias']['aliasname']]
            ###

            self.aliasname2fromtable[alias] = table
            self.aliasname2fullname[alias] = table.getFullName()

        self.aliasnames = set(self.aliasname2fromtable.keys())

        # Comparison被修改过，遇到tpch这种会提取alias，例如从l_orderkey中提取l
        ...
        for comparison in self.comparison_list:
            if len(comparison.aliasname_list) == 2:
                ...
                self.alias_selectivity[self.aliasname2id[left_aliasname]] = self.alias_selectivity[self.aliasname2id[
                    left_aliasname]] + self.pgrunner.getSelectivity(
                ...
                self.count_selectivity[getColumnId(comparison.column)] = self.count_selectivity[getColumnId(
                    comparison.column)] + self.pgrunner.getSelectivity(
                    str(self.aliasname2fromtable[comparison.aliasname_list[0]]), str(comparison))
...
class TreeBuilder:
    ...
    def __alias_name(self, node):
        if "Alias" in node:
            ###
            if config.database == 'tpch':
                return np.asarray([self.aliasname2id[config.tpch_map[node["Alias"]]]])
            ###
