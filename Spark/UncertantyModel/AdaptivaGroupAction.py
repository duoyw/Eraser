...
from Plan import PlanNode, FilterPlanNode, ProjectPlanNode, JoinPlanNode
from UncertantyModel.AdaptivaGroupTree import TreeNode
from UncertantyModel.PlansManeger import PlansManager
from model_config import db_node_config, GroupEnable
...
class Action:
    ...
    def score_if_split(self, tree_node: TreeNode):
        left_tree_node, right_tree_node = self._split_no_add_action(tree_node)
        return self._score(left_tree_node, right_tree_node)
...
class TableTypeAction(OnceAction):
    ...
    def get_cur_value(self, plan_node: PlanNode):
        return plan_node.get_node_type()
