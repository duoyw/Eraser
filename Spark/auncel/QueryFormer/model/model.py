...
from auncel.utils import to_one_hot_tensor


def cal_one_hot_feature_dim(encoding: Encoding, hist_embed_size):
    type_embed_size = encoding.get_type_size()
    join_embed_size = encoding.get_join_size()
    col_embed_size = encoding.get_col_size()
    op_embed_size = encoding.get_filter_op_size()
    table_embed_size = encoding.get_table_size()
    return type_embed_size + join_embed_size + col_embed_size + op_embed_size + table_embed_size + 1 + hist_embed_size
...
class FeatureEmbed(nn.Module):
    def __init__(self, embed_size=32, tables=10, types=20, joins=100, columns=50,
    ...
    def getTable(self, table_sample):
        ...
        if self.use_sample:
            emb += self.linearSample(sample.double())
        return emb
    ...
    def getHist(self, hists, filtersMask):
        ...
        emb = self.linearHist(histExpand.double())
...
class FeatureEmbedOneHot(nn.Module):
    def __init__(self, encoding: Encoding, embed_size=32, tables=10, types=20, joins=100, columns=50,
                 ops=4, use_sample=True, use_hist=True, bin_number=50, ):
        super(FeatureEmbedOneHot, self).__init__()
        ...
        total_size = cal_one_hot_feature_dim(self.encoding, embed_size)
        self.linearSample = nn.Linear(1000, embed_size)

        self.linearHist = nn.Linear(bin_number, embed_size)

        if not use_hist:
            total_size = total_size - embed_size
        self.project = nn.Linear(total_size, total_size)

    def get_one_hot_embed_size(self):
        encoding = self.encoding
        return encoding.get_type_size(), encoding.get_join_size(), encoding.get_col_size(), \
               encoding.get_filter_op_size(), encoding.get_table_size()
    ...
    def getType(self, typeId):
        emb = to_one_hot_tensor(typeId, self.encoding.get_type_size())
        return emb.squeeze(1)

    def getTable(self, table_sample):
        table, sample = torch.split(table_sample, (1, 1000), dim=-1)
        emb = to_one_hot_tensor(table, self.encoding.get_table_size())
        if self.use_sample:
            raise RuntimeError
        return emb.squeeze(1)

    def getJoin(self, joinId):
        emb = to_one_hot_tensor(joinId, self.encoding.get_join_size())
        return emb.squeeze(1)
    ...
    def getFilter(self, filtersId, filtersMask):
        ...
        col = to_one_hot_tensor(colsId, self.encoding.get_col_size())
        op = to_one_hot_tensor(opsId, self.encoding.get_filter_op_size())
        vals = vals.double()
        concat = torch.cat((col, op, vals), dim=-1)
        # concat = F.leaky_relu(self.linearFilter(concat))
        # concat = F.leaky_relu(self.linearFilter2(concat))
...
class QueryFormer(nn.Module):
    def __init__(self, emb_size=32, ffn_dim=32, head_size=8,
                 ...
                 pred_hid=256, encoding: Encoding = None,use_one_hot=False
                 ):
        super(QueryFormer, self).__init__()


        if use_one_hot:
            self.embed_layer = FeatureEmbedOneHot(encoding=encoding, embed_size=emb_size, use_sample=use_sample,
                                                  use_hist=use_hist, bin_number=bin_number)
            hidden_dim = cal_one_hot_feature_dim(encoding, emb_size)
        else:
            self.embed_layer = FeatureEmbed(emb_size, use_sample=use_sample, use_hist=use_hist, bin_number=bin_number)
            hidden_dim = cal_hidden_dim(emb_size, use_hist)
    ...
    def forward(self, batched_data):
        ...
        attn_bias = batched_data[0]
        rel_pos = batched_data[1]
        x = batched_data[2]
        heights = batched_data[3]
        ...
        node_feature = self.embed_layer(x_view).view(n_batch, -1, self.hidden_dim)
...
def cal_hidden_dim(emb_size, use_hist):
    if use_hist:
