...
from Common.EncordCollector import encord_collector
...
class DynamicPooling(nn.Module):
    def forward(self, x):
        res = torch.mean(x[0], dim=2)
        encord_collector.add(res)
        return res
        # return torch.max(x[0], dim=2).values

        # return torch.mean(x[0], dim=2)
        # return torch.max(x[0], dim=2).values

# class ConnectPooling(nn.Module):
#     def __init__(self, ):
#         self.activation = activation
#     def forward(self, x):
#         return torch.max(x[0], dim=2).values
